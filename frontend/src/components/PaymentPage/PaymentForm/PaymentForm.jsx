...
const PaymentForm = ({ game }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    await api
      .post(`library/library/add/${user.user_id}/${game.id}/`)
      .then((response) => {
        console.log(response.data);
      })
      .catch((error) => {
    ...
    navigate(`/user/id/${user.user_id}/library/page/1`);
  };
      ...
      {game.price_in_cents !== 0 && (
        <div className="card-panel">
          ...
          <div className="card-form-container">
            <CardFormElement
              CardComponent={CardNumberElement}
              label={"Номер карты"}
            />
            <div className="card-form-row">
              <CardFormElement
                CardComponent={CardExpiryElement}
...
const CardFormElement = ({ CardComponent, label }) => {
  ...
  return (
    <div className="card-container">
      ...
      <div className={`card-errors${error ? " visible" : ""}`} role="alert">
        {error}
      </div>
    </div>
  );
